...
fn parse_hands(input:&String) -> (Vec<usize>,Vec<usize>){
...
fn recursive_round(p1:&mut Vec<usize>, p2:&mut Vec<usize>, hist:&mut Vec<u64>) -> (usize,usize,usize){
    let h64 = hash((&p1,&p2));
    if hist.contains(&h64){ return (1,1,3) } else { hist.push(h64) };
    ...
    if p1.len()>=card_p1 && p2.len()>=card_p2{
        let mut hist2 = vec![];
        let mut rec_p1:Vec<usize> = p1[p1.len()-card_p1..p1.len()].to_vec();
        let mut rec_p2:Vec<usize> = p2[p2.len()-card_p2..p2.len()].to_vec();
